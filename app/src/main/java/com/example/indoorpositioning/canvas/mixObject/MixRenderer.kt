...
class MixRenderer : CustomRenderer(), GLSurfaceView.Renderer {
    ...
    override fun onDrawFrame(p0: GL10?) {
        ...
        while ( i< lstPoint.size){
            if(lstPoint[i].getIsBelongCurve()){
                var j = i + 1
                while( j < lstPoint.size){
                    if(lstPoint[j].getIsBelongCurve())
                        ++j
                    else break
                }
                var vers = lstPoint.subList(i,j)
                if(vers.size > 1)
                    shader!!.drawACurve(lstPoint.subList(i,j),ColorHelper().convertColorToFloatArray(borderColor))
                i = j
        ...
        if(currentActionPosition < history.size){
            ...
            when(handleAction.getAction()){
                HistoryAction.HistoryActionType.ADD ->{
                    ...
                    if(!handleAction.getMainObject().getIsBelongCurve()){
    ...
    override fun handleUndoAction(){
        if(currentActionPosition != 0){
            ...
            when(handleAction.getAction()){
                HistoryAction.HistoryActionType.ADD -> {
                    Log.d("== UNDO MIX","ADD")
                    //remove related line if it does not belong any curve
                    if(!handleAction.getMainObject()!!.getIsBelongCurve()){
                        var i = 0
                        while(i < lstLine.size){
                ...
                HistoryAction.HistoryActionType.DELETE -> {
                    Log.d("== UNDO MIX","DELETE")
                    var i = 0
                    var position : Int? = null
                    if(handleAction.getRelatedObject() != null){
                        while( i < lstPoint.size){
                            if(lstPoint[i].getId() == handleAction.getRelatedObject()!!.getId()){
                                position = i
                                break
                            }
                            else
                    ...
                    if(position != null){
                        Log.d("== MIX RENDERER position","${position}")
                        lstPoint.add(position + 1, handleAction.getMainObject())
                    }
                    else{
                        lstPoint.add(handleAction.getMainObject())
                    }
